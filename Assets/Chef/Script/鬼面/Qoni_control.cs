...
public class Qoni_control : MonoBehaviour
{
    ...
    public void Face_reset()
    {
        ...
        for (int i = 0; i < Face.Length; i++)
        {
            if (Face_obj[i] != null)
            {
                Face_obj[i].GetComponent<SpriteRenderer>().sprite=Face_obj[i].GetComponent<Qoni_image>().Face_image[Face[i]];
            }
        ...
        if (finish)
        {
            for(int i=0;i< Face_C.Length; i++) {
                Face_C[i].GetComponent<Qoni_Face_change>().locking = true;
            }
            Q_mode = 1;
            delay_mode = 0;
            delay = 2;
